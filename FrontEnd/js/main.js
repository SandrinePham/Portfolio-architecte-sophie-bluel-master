...
function afficherFiltres(data) {
  ...
  data.forEach(
    (projet) => projet.category && categoriesSet.add(projet.category.name)
  );
  ...
  [...categoriesSet].forEach((category) => {
    ...
    li.addEventListener("click", () => {
      ...
      const filteredData =
        category === "Tous"
          ? data
          : data.filter((projet) => projet.category.name === category);
  ...
  fetch("http://localhost:5678/api/works")
    ...
    .then((data) => {
      ...
      document
        ...
        .forEach((btn) => btn.addEventListener("click", deleteProject));
    });
}
